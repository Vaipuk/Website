...
interface LinkTileProps {
    platform: 'Twitter' | 'GitHub' | 'LinkedIn' | 'Instagram' | 'Blog';
    url: string;
    username?: string;
    label?: string; // Custom label override
    icon?: React.ReactNode;
    isInternalLink?: boolean; // For internal routing
}
...
export const LinkTile: React.FC<LinkTileProps> = ({
    ...
    return (
        <Tile colSpan={1} rowSpan={1} onClick={handleClick} className={`${styles.linkTile} ${platformClass}`}>
            <div className={styles.iconContainer}>
                {icon || getDefaultIcon(platform)}
            </div>
            <div className={styles.info}>
                <div className={styles.platform}>{label || platform}</div>
